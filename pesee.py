...
with st.form("form_poids", clear_on_submit=True):
    ...
    poids = st.number_input("Poids (kg)", min_value=0.0, max_value=150.0, step=0.1, format="%.1f", value=None)    
    components.html(
        """
        <script>
    ...
    if submitted:
        if nom_joueur != "" and poids is not None and poids > 40.0:
            df = pd.read_csv(fichier_poids, sep=None, engine="python")
...
df_histo = pd.read_csv(fichier_poids, sep=None, engine="python")
# Standardisation du format de date (JJ/MM/AAAA vers AAAA-MM-JJ)
df_histo["Date"] = pd.to_datetime(df_histo["Date"], dayfirst=True, errors="coerce").dt.date.astype(str)
...
with st.expander("⚙️ Administration avancée (Édition / Restauration)"):
    ...
    st.divider()
    st.markdown("### Restauration d'une sauvegarde")
    fichier_upload = st.file_uploader("Importer un fichier historique_poids_sdr.csv", type=["csv", "txt", "tsv"])
    
    if fichier_upload is not None:
        if st.button("🔄 Remplacer les données actuelles par ce fichier", use_container_width=True):
            df_upload = pd.read_csv(fichier_upload, sep=None, engine="python")
            df_upload["Date"] = pd.to_datetime(df_upload["Date"], dayfirst=True, errors="coerce").dt.date.astype(str)
            df_upload.to_csv(fichier_poids, index=False)
            st.success("Données restaurées avec succès ! L'application va se mettre à jour.")
            st.rerun()
